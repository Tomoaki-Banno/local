...
class Partner_DataLock_Lock
{

    function execute(&$form)
    {
        // タブ、マイメニュー用。この画面では画面表示にも使用
        $form['gen_pageTitle'] = _g("購買データロック");
        $form['gen_pageHelp'] = _g("データロック");
        $form['addMsg'] = _g("※ すべてのデータをロックする場合は、[メンテナンス] - [データロック] 画面で処理を行ってください。");

        $form['bgColor1'] = '#d5ebff';
        $form['target1'] = _g("対象：　[購買管理]メニューのデータ");
        $form['ajaxAction1'] = 'Partner_DataLock_AjaxLock';

        $date = Logic_SystemDate::getBuyLockDate();
