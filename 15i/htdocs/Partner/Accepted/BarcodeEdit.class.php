...
class Partner_Accepted_BarcodeEdit extends Base_EditBase
{
    ...
    function setViewParam(&$form)
    {
        ...
        $form['gen_pageTitle'] = _g("バーコード注文受入登録");
        $form['gen_entryAction'] = "Partner_Accepted_BarcodeEntry&show_line_number={$form['show_line_number']}";
        ...
        $form['gen_javascript_noEscape'] = "
            ...
            function onOrderNoChange(no) {
                ...
                gen.ajax.connect('Partner_Accepted_AjaxOrderParamBarcode', {order_no : orderNo}, 
                    function(j) {
                        if (j != '') {
            ...
            function entry() {
               ...
               for (var i=1;i<={$line_count};i++) {
                   ...
                   if (!gen.util.isNumeric(qty) && !(no == '' && qty == '')) {
                       str = '{$msg4}'; alert(str.replace('%1', (i + " . h($form['show_line_number']) . " - 1))); $('#quantity_'+i).select(); return;
                   }
                   if (no == '' && qty != '') {
                       str = '{$msg6}'; alert(str.replace('%1', (i + " . h($form['show_line_number']) . " - 1))); $('#order_no_'+i).select(); return;
                   }
                   if (no != '') {
                       cnt++;
                   }
        ...
        $html_location_id = Gen_String::makeSelectHtml("location_id", $opt, @$form['location_id'], "", "Partner_Accepted_BarcodeEdit", @$form['gen_pins']);
