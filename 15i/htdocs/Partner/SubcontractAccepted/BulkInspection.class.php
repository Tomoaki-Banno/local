...
class Partner_SubcontractAccepted_BulkInspection
{
    ...
    function validate(&$validator, &$form)
    {
        ...
        return 'action:Partner_SubcontractAccepted_List';        // if error
    }
    ...
    function execute(&$form)
    {
        ...
        if ($form['gen_readonly'] == 'true') {
            return "action:Partner_SubcontractAccepted_List";
        }
        ...
        // データアクセスログ
        Gen_Log::dataAccessLog(_g("外製受入登録"), _g("一括検収"), _g("[検収日] ") . ($inspectionDate != "" ? date('Y-m-d', strtotime($inspectionDate)) : _g("削除")));
