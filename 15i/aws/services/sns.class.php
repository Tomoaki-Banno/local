...
class AmazonSNS extends CFRuntime
{
	...
	public function unsubscribe($subscription_arn, $opt = null)
	{
        ...
        // ***** Gen Hack *****
        // endPoint作成APIを追加（AWS SDK for PHP 2.xでは実装されているもの）
	public function create_platform_endpoint($application_arn, $token, $opt = null)
	{
		if (!$opt) $opt = array();
		$opt['PlatformApplicationArn'] = $application_arn;
		$opt['Token'] = $token;
                $this->disable_ssl_verification();  // winで実行したときのcurlエラー対策
		
		return $this->authenticate('CreatePlatformEndpoint', $opt);
        }

        // ***** Gen Hack *****
        // endpoint_arnに対して(topic_arnではなく)publishできるAPIを追加（AWS SDK for PHP 2.xでは実装されているもの）
	public function publish_for_endpoint($endpoint_arn, $message, $opt = null)
	{
		if (!$opt) $opt = array();
		$opt['TargetArn'] = $endpoint_arn;
		$opt['Message'] = $message;
                $this->disable_ssl_verification();  // winで実行したときのcurlエラー対策
